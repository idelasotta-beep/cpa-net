  ...
  try {
    ...
    for (const lead of leads) {
      ...
      if (result.ok) {
        ...
        ok++;
        log.info({ leadId: lead.id, phone: maskPhone(lead.customerPhone), networkLeadId: result.networkLeadId }, "lead enviado");
      } else {
    ...
    return NextResponse.json({ processed: leads.length, ok, failed, skipped });
  } catch (err) {
    log.error(
